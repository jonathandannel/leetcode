...
# The second pair [3,4] means we have 
# freq = 3 and val = 4 so we generate [4,4,4].
# At the end the concatenation 
# [2] + [4,4,4] is [2,4,4,4].

def decompressRLElist(self, nums: [int]) -> [int]:
    master_list = []
    input_len = len(nums)
    if input_len > 100 or input_len < 2:
        return []
    if input_len % 2 != 0:
        return []
